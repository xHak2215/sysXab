modul_neim=['pip', 'beautifulsoup4' ,'tkinter-page' ,'requests', 'wikipedia psutil','requests''tabulate','you-get','wmi','asyncio','PyQt5','getch','subprocess.run','QtGui','QtGui','sockets','folium','pyfiglet','speedtest-cli']
kast = '>>'
try:
    import subprocess
    import os
...
#def tablicaa(strs1,strs2,strs3):
#    tablihka=f'       _/_                                      _\ \n ___/__/  {strs1}  \__\__ \n | « « |            {strs2}             | » » | \n ¯¯\¯¯\      {strs3}     /¯¯/¯¯ \n ¯\¯                                      ¯/ \n ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯'
#    print(tablihka)
...
def test_net():
    ...
    print(f"Скорость скачивания: {download} mb") 
    print(f"Скорость закачки: {upload} mb")
    #print(f"ping {ping}")
    

def binary_encoder(message):
    encoded_message = ""
    for char in message:
        encoded_char = format(ord(char), '08b')  # Преобразуем символ в его двоичное представление с помощью функции ord и форматирования
        encoded_message += encoded_char + " "  # Добавляем зашифрованный символ к зашифрованному сообщению с пробелом
    return encoded_message

def binary_decoder(encoded_message):
    decoded_message = ""
    binary_values = encoded_message.split()  # Разделяем зашифрованные символы по пробелам
    for binary_value in binary_values:
        decoded_char = chr(int(binary_value, 2))  # Преобразуем двоичное значение обратно в символ с помощью функции chr
        decoded_message += decoded_char  # Добавляем расшифрованный символ к расшифрованному сообщению
    return decoded_message

message = "Hello, world!"
encoded_message = binary_encoder(message)
print("Encoded message:", encoded_message)

decoded_message = binary_decoder(encoded_message)
...
while True:
    ...
    command = input(f'{app}{os.getcwd()} {adm} "sh" {kast}')
    ...
    if command == "exit":
        ...
        if lang == "ru":
            ...
            print(' "dir" - (show the contents of the current directory),\f,"open" - opens files (not all) ')
            print("cm <directory name> - (create directory), \f del <directory name> - (delete directory). \f pwd - ticking repository \f games - killing time \f install Winterm - installing Windows terminal ")
...
#        subprocess.call(["ping", "-c", "3", "google.com"])
    ...
    #    install_with_apt('git')
        ...
        key = command.split(" ")[1]
        pyti = command.split(" ")[2]
        if key == "jpg":
            if os.path.isfile(pyti):
                webbrowser.open(pyti)
            else:
                print("error file 1")
        elif key == 'txt':
            if os.path.isfile(pyti):
                with open(pyti, 'r') as file:
                    print(file.read())
            else:
                print("error file 1")
        elif key == 'bat':
            if os.path.isfile(pyti):
                subprocess.Popen(pyti, shell=True)
            else:
                print("error file 1")
        else:
            print("error file 1")
        ...
        os.system('cd C:/Windows/System32/conhost.exe')
    elif command.startswith("binout"):
        encoded_message= command.split(" ")[1]
        print(binary_decoder(encoded_message))
    elif command.startswith("binin"):
        message= command.split(" ")[1]
        print(binary_encoder(message))
       ...
       with open("command.txt", "r") as f: 
        ...
        try:
            ...
            if defi == "kast":
                ...
                try:
                    ...
                    with open('start direct.txt', 'w') as file: 
        ...
        subprocess.Popen(f'powershell.exe','{command}')
    elif command == "py.net":
        ip_py()
    elif command == "test internet":
        test_net()
    elif command == "pip uninstall":
        pak = input(f"модуль{kast}")
        os.system(f'pip install {pak}')
